...
public class IndexScroller {
	...
	public IndexScroller(Context context, ListView lv) {
		...
		// TODO: no margin according to design
	...
	public void draw(Canvas canvas) {
		...
		if (mSections != null && mSections.length > 0) {
			...
			indexPaint.setTypeface(SFonts.getInstance(mContext).getFont(
					SFonts.FONT_VOLK_SWAGAN_SERIAL_XBOLD));
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			// TODO: If down event occurs inside index bar region, start indexing
			if (mState != STATE_HIDDEN && contains(ev.getX(), ev.getY())) {
				setState(STATE_SHOWN);
				// TODO: It demonstrates that the motion event started from index bar
				mIsIndexing = true;
				// TODO: Determine which section the point is in, and move the list to that section
		...
		case MotionEvent.ACTION_MOVE:
			if (mIsIndexing) {
				// TODO: If this event moves inside index bar
	...
	private void setState(int state) {
		...
		switch (mState) {
		case STATE_HIDDEN:
			// TODO: Cancel any fade effect
			mHandler.removeMessages(0);
			break;
		case STATE_SHOWING:
			// TODO: Start to fade in
			mAlphaRate = 0;
			fade(0);
			break;
		case STATE_SHOWN:
			...
			break;
		case STATE_HIDING:
			// TODO: Start to fade out after three seconds
			mAlphaRate = 1;
			fade(3000);
			break;
		}
	...
	private boolean contains(float x, float y) {
		// TODO: Determine if the point is in index bar region, which includes the right margin of the bar
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case STATE_SHOWING:
				...
				break;
			case STATE_SHOWN:
				// TODO: If no action, hide automatically
				setState(STATE_HIDING);
				break;
			case STATE_HIDING:
